...
struct node_gra *CreatePoissonRandomGraph(int S,int L)
{
	...
	for(i=0;i<L;i++){			//Situem els L links
		do{
			...
			control=AddAdjacency(list[des1],des2,0);		//I els enllaça
...
struct node_gra *CreateInternetModel(int n,double r)
	...
	//  p->state_in ... # links q falten per enllaçar
{
	int i,j,k,l;
	int dest,orig,ant;
	struct node_gra *root=NULL;
	...
	//Creem la capçalera de la nostra xarxa
	root=CreateHeaderGraph();
	...
	for(i=2;i<n;i++){
		...
		//Calculem la posició x,y del nou node. Mirem si cal fer un salt o si continuem el mateix cami
		...
		//O be que queden 0 o 1 links per enllaçar, o be en queden mes pero tots pertanyen al mateix node
		final=0; control=0;
		fprintf(fppf,"%i\t%i\n",(int) num_links,(int) num_links_pendents);
		while(num_links_pendents>=2 && !final)
		{
			//Comprovem q hi hagi mes d'un node amb links per enllaçar
			...
			if(!final) //Es pot fer l'enllaç pq hi ha 2 o mes nodes per enllaçar.
			{
				...
				if(prob_rand<prob)
				{
					...
					//I mirem si es poden fer mes enllaços amb probabilitat r
					prob_rand=ran2(&seed);
